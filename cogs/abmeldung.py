...
class AbmeldungCog(commands.Cog):
    ...
    # Triggered ein sofortiges Update des TeamlisteCog-Embeds
    async def trigger_teamlist_update(self, guild: discord.Guild):
        if not guild:
            return
        ...
        if guild:
            ...
            if member:
                ...
                if role_id:
                    ...
                    if role and role in member.roles:
                        try:
                            ...
                            print(
                                "⚠️ Keine Rechte, um Rolle von"
                                f" {member.display_name} zu entfernen."
                            )
                ...
                try:
                    ...
                    print(
                        "⚠️ Keine Rechte, um den Namen von"
                        f" {member.display_name} zu ändern."
                    )
    ...
    async def check_expired_abmeldungen(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT user_id, user_name, grund, bis, original_nick, guild_id"
            " FROM abmeldungen"
        )
        ...
        if expired_users:
            print(
                f"🧹 {len(expired_users)} abgelaufene Abmeldung(en)"
                " automatisch entfernt."
            )
            await self.update_live_list()

            # Teamlisten der betroffenen Server aktualisieren
    ...
    async def abmeldung(
        ...
        try:
            ...
            await interaction.response.send_message(
                "❌ **Ungültiges Startdatumsformat!** Bitte benutze genau das"
                " Format `TT.MM.JJJJ` (z. B. `20.09.2026`).",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            await interaction.response.send_message(
                "❌ **Ungültiges Enddatumsformat!** Bitte benutze genau das"
                " Format `TT.MM.JJJJ` (z. B. `25.09.2026`).",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            print(
                f"⚠️ Bot hat keine Rechte, den Namen von {interaction.user.name}"
                " zu ändern (z. B. Server-Owner/höhere Rolle)."
            )
        except discord.HTTPException as e:
            print(f"⚠️ Namensänderung fehlgeschlagen: {e}")
        ...
        await interaction.response.send_message(
            f"✅ Deine Abmeldung vom **{von_formatted}** bis zum"
            f" **{bis_formatted}** wurde eingetragen.",
            ephemeral=True,
        )
        await self.update_live_list()
    ...
    async def anmeldung(self, interaction: discord.Interaction):
        ...
        if row:
            ...
            await interaction.response.send_message(
                "👋 Willkommen zurück! Du wurdest aus der Liste entfernt und"
                " dein Name wurde zurückgesetzt.",
                ephemeral=True,
            )
            await self.update_live_list()
            await self.trigger_teamlist_update(interaction.guild)
